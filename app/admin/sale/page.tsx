...
import {
  Zap, Save, Clock, Eye, Package, Search, RefreshCw,
  CheckCircle, AlertCircle, Flame, ToggleLeft, ToggleRight, Tag, X,
} from "lucide-react";
...
export default function AdminSalePage() {
  ...
  const handleSaveConfig = async () => {
    if (config.isActive && !config.endsAt) { showToast("error", "Please set a sale end date/time before activating"); return; }
    if (config.isActive && config.endsAt && new Date(config.endsAt).getTime() <= Date.now()) { showToast("error", "Sale end time must be in the future"); return; }
    setIsSaving(true);
    try {
      const res = await fetch("/api/admin/sale", {
    ...
    <div className="space-y-6 p-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-gray-500 text-sm mt-1">Configure the sale hero and tag products to appear in the store</p>
        </div>
        ...
        <div className="xl:col-span-1 space-y-4">
          ...
          <Card className="p-5 border-0 shadow-md">
            <div className="flex items-start justify-between gap-4">
              <div>
                ...
                <p className="text-xs text-gray-500">Enable to show the flash sale page and countdown</p>
              </div>
              <button onClick={() => setConfig((prev) => ({ ...prev, isActive: !prev.isActive }))} className="shrink-0 mt-0.5 focus:outline-none">
          ...
          <Card className="p-5 border-0 shadow-md">
            <div className="flex items-center justify-between mb-3">
              <h2 className="font-bold text-gray-900 flex items-center gap-2"><Tag className="h-4 w-4 text-green-600" /> In Flash Sale</h2>
              <span className="text-xs font-black bg-yellow-100 text-yellow-700 px-2.5 py-1 rounded-full">{flashProducts.length} items</span>
            </div>
            {flashProducts.length === 0 ? (
              <div className="text-center py-6 border-2 border-dashed border-gray-100 rounded-xl">
                <Zap className="h-6 w-6 text-gray-300 mx-auto mb-2" />
                <p className="text-xs text-gray-400">No products tagged yet.<br />Click products on the right to add.</p>
              </div>
            ) : (
              <div className="space-y-2 max-h-60 overflow-y-auto">
                {flashProducts.map((p) => (
                  <div key={p._id} className="flex items-center gap-3 bg-yellow-50 border border-yellow-100 rounded-xl px-3 py-2">
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <p className="text-xs text-green-700 font-bold">
                        ...
                        {p.discount > 0 && <span className="text-gray-400 font-normal ml-1">(was {p.retailPrice})</span>}
                      </p>
                    </div>
                    <button onClick={() => toggleFlashSale(p)} disabled={updatingId === p._id} className="text-gray-300 hover:text-red-500 transition-colors shrink-0 disabled:opacity-50" title="Remove from flash sale">
                      {updatingId === p._id ? <RefreshCw className="h-4 w-4 animate-spin text-green-500" /> : <X className="h-4 w-4" />}
                    </button>
                  </div>
                ))}
              </div>
        ...
        <div className="xl:col-span-2">
          <Card className="p-5 border-0 shadow-md h-full">
            <div className="flex items-center justify-between mb-1">
              ...
              <span className="text-xs text-gray-400 bg-gray-100 px-3 py-1 rounded-full">Click to add / remove from sale</span>
            </div>
            <p className="text-xs text-gray-400 mb-4">
              Products marked as <code className="bg-yellow-50 text-yellow-700 px-1.5 py-0.5 rounded font-mono">Flash Sale</code> appear on the sale page
            </p>
