            ...
            <div className="p-6">
              <div className="flex justify-between items-start mb-4">
                ...
                <div className="flex gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                  ...
                  <Button variant="ghost" size="icon" onClick={() => handleDelete(supplier._id)} className="h-8 w-8 text-red-500">
              ...
              <div className={`p-4 rounded-xl mb-4 ${(supplier.balance ?? 0) > 0 ? "bg-red-50 border border-red-100" : "bg-green-50 border border-green-100"}`}>
                <p className="text-[10px] uppercase font-bold text-gray-500 tracking-wider">Remaining to Pay</p>
          ...
          <Card className="w-full max-w-4xl max-h-[90vh] overflow-hidden flex flex-col shadow-2xl">
            <div className="p-6 border-b flex justify-between items-center bg-slate-50">
              <div>
                ...
                <p className="text-sm text-slate-500">Statement of Account & Purchase Ledger</p>
              </div>
              <Button variant="ghost" size="icon" onClick={() => setIsHistoryOpen(false)}><X /></Button>
            </div>
                ...
                <tbody className="divide-y">
                  {selectedSupplier.purchases?.map((p) => (
                    <tr key={p._id} className="text-sm hover:bg-slate-50 transition-colors">
                      <td className="py-4 text-gray-600">{format(new Date(p.createdAt), "dd MMM yyyy")}</td>
                      <td className="py-4 font-mono font-bold">{p.supplierInvoiceNo || "N/A"}</td>
                      <td className="py-4 font-semibold">Rs {formatCurrency(p.totalAmount)}</td>
                      <td className="py-4 text-green-600 font-medium">(-) Rs {formatCurrency(p.amountPaid)}</td>
                      <td className={`py-4 text-right font-bold ${(p.balanceDue ?? 0) > 0 ? "text-red-500" : "text-gray-900"}`}>
            ...
            <div className="p-6 bg-slate-900 text-white flex justify-between items-center">
              <span className="text-lg opacity-80">Total Outstanding Balance:</span>
              <span className="text-3xl font-black">Rs {formatCurrency(selectedSupplier.balance)}</span>
            </div>
          </Card>
        </div>
      )}
