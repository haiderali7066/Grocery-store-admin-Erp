...
> = {
  ...
  cheque: {
    ...
    walletKey: null,
    icon: <CreditCard className="h-3.5 w-3.5" />,
    pill: "text-gray-700 bg-gray-50 border-gray-200",
  },
};
...
function WalletBadge({
  ...
  if (!cfg.walletKey)
    return (
      <p className="text-xs text-gray-400 mt-1">
        No wallet tracking for cheques
      </p>
    );
  ...
  return (
    <div
      className={`mt-1.5 flex items-center gap-1.5 px-2.5 py-1 rounded-lg border text-xs font-semibold w-fit ${insufficient ? "bg-red-50 border-red-200 text-red-700" : cfg.pill}`}
    >
      {cfg.icon}
...
export default function InventoryPage() {
  ...
  const calculateUnitCost = (item: PurchaseProduct) => {
    const base = parseFloat(item.buyingRate) || 0,
      tax = parseFloat(item.taxValue) || 0,
      freight = parseFloat(item.freightPerUnit) || 0;
    return (
  ...
  const calculateProfit = (item: PurchaseProduct) => {
    ...
    return {
      profitPerUnit,
      totalProfit,
      margin: unitCost > 0 ? (profitPerUnit / unitCost) * 100 : 0,
    };
  };
  ...
  const newWalletBal =
    wallet && newMethodCfg.walletKey
      ? (wallet[newMethodCfg.walletKey] as number)
      : null;
      ...
      <div className="flex flex-col gap-4">
                    ...
                    <div>
                      ...
                      <select
                        ...
                        onChange={(e) =>
                          setPurchaseData({
                            ...purchaseData,
                            paymentMethod: e.target.value as PayMethod,
                          })
                        }
                        className="w-full p-2 border rounded-md bg-white"
                      >
                        {(
                          Object.entries(PAY_CFG) as [
                            PayMethod,
                            (typeof PAY_CFG)[PayMethod],
                          ][]
                        ).map(([k, c]) => (
                  ...
                  <Card className="p-4 border-orange-200 bg-orange-50/50">
                    ...
                    <div>
                      ...
                      <Input
                        ...
                        className={`text-lg font-bold bg-white ${insufficientNew ? "border-red-400" : "border-orange-300"}`}
                        min="0"
                      />
                    </div>
                    ...
                    {newWalletBal !== null && amountPaidNum > 0 && (
                      <div
                        className={`mt-3 p-3 rounded-xl border text-sm space-y-1 ${insufficientNew ? "bg-red-50 border-red-200" : "bg-green-50 border-green-200"}`}
                      >
                        <div className="flex justify-between">
                          <span className="text-gray-600">
                            {newMethodCfg.label} balance now:
                          </span>
                        ...
                        <div className="flex justify-between">
                          ...
                          <span
                            className={`font-black ${newWalletBal - amountPaidNum < 0 ? "text-red-600" : "text-green-700"}`}
                          >
                            Rs {fmt(newWalletBal - amountPaidNum)}
                          </span>
                        </div>
                        {insufficientNew && (
                          <p className="text-red-600 text-xs font-semibold pt-0.5">
                            ⚠ Wallet will go negative — you'll be warned before
                            submitting
                          </p>
                        )}
                      </div>
        ...
        {wallet && (
          <div className="flex flex-wrap gap-2">
            <span className="text-xs text-gray-500 font-semibold self-center mr-1">
              Wallet:
            </span>
            {(
              Object.entries(PAY_CFG) as [
              ...
              .map(([key, c]) => {
                const bal = wallet[
                  c.walletKey as keyof WalletBalances
                ] as number;
                return (
                  <div
                    key={key}
                    className={`flex items-center gap-1.5 px-3 py-1.5 rounded-full border text-xs font-bold ${c.pill}`}
                  >
            ...
            <div className="flex items-center gap-1.5 px-3 py-1.5 rounded-full border text-xs font-bold text-slate-700 bg-slate-50 border-slate-200">
              <WalletIcon className="h-3.5 w-3.5" /> Total: Rs{" "}
              {fmt(wallet.totalBalance)}
            </div>
          </div>
        )}
      </div>
                    ...
                    return (
                      ...
                      >
                        ...
                        <td className="px-5 py-4">
                          {mCfg ? (
                            <span
                              className={`inline-flex items-center gap-1 px-2 py-1 rounded-full text-xs font-bold border ${mCfg.pill}`}
                            >
                              {mCfg.icon}
                              {mCfg.label}
                            </span>
                        ...
                        <td className="px-5 py-4">
                          <span
                            className={`px-2.5 py-1 rounded-full text-xs font-bold uppercase ${p.paymentStatus === "completed" ? "bg-green-100 text-green-700" : p.paymentStatus === "partial" ? "bg-yellow-100 text-yellow-700" : "bg-red-100 text-red-700"}`}
                          >
                            {p.paymentStatus}
                          </span>
                        </td>
        ...
        <div className="fixed inset-0 z-50 bg-black/50 flex items-center justify-center p-4">
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-lg p-6">
            ...
            <div className="bg-gray-50 rounded-xl p-4 mb-5 text-sm space-y-1">
              ...
              <p className="text-xs text-gray-400 mt-1">
                Only payment details can be edited. To change products, delete
                and recreate the purchase.
              </p>
            </div>
            ...
            <div className="space-y-4">
              ...
              <div>
                ...
                <WalletBadge
                  method={editForm.paymentMethod as PayMethod}
                  wallet={wallet}
                  paying={parseFloat(editForm.amountPaid) || 0}
                />
              </div>
              <div>
                <label className="block text-xs font-bold text-gray-500 uppercase tracking-widest mb-1.5">
