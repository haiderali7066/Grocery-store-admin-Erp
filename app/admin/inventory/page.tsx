...
function Pagination({
  currentPage, totalPages, onPageChange, totalItems, itemsPerPage, label = "items",
}: {
  currentPage: number; totalPages: number; onPageChange: (page: number) => void;
  totalItems?: number; itemsPerPage?: number; label?: string;
}) {
  ...
  return (
    <div className="bg-gray-50 px-5 py-4 border-t border-gray-100 flex items-center justify-between no-print flex-wrap gap-3">
      ...
      <div className="flex items-center gap-1">
        <button onClick={() => onPageChange(1)} disabled={currentPage === 1} className="px-2 py-1.5 rounded-lg border border-gray-200 text-xs text-gray-500 hover:bg-gray-100 disabled:opacity-40 disabled:cursor-not-allowed font-bold">«</button>
        <button onClick={() => onPageChange(currentPage - 1)} disabled={currentPage === 1} className="p-1.5 rounded-lg border border-gray-200 text-gray-600 hover:bg-gray-100 disabled:opacity-40 disabled:cursor-not-allowed">
        ...
        {getPageNumbers().map((page, idx) =>
          page === "..." ? (
            ...
            <button key={page} onClick={() => onPageChange(page as number)}
              className={`px-3 py-1.5 rounded-lg text-xs font-bold transition-all min-w-[32px] ${page === currentPage ? "bg-blue-600 text-white shadow-sm" : "bg-white border border-gray-200 text-gray-600 hover:bg-gray-50"}`}>
              {page}
            </button>
          )
        )}
        <button onClick={() => onPageChange(currentPage + 1)} disabled={currentPage === totalPages} className="p-1.5 rounded-lg border border-gray-200 text-gray-600 hover:bg-gray-100 disabled:opacity-40 disabled:cursor-not-allowed">
          <ChevronRight className="h-4 w-4" />
        </button>
        <button onClick={() => onPageChange(totalPages)} disabled={currentPage === totalPages} className="px-2 py-1.5 rounded-lg border border-gray-200 text-xs text-gray-500 hover:bg-gray-100 disabled:opacity-40 disabled:cursor-not-allowed font-bold">»</button>
      </div>
    </div>
  );
}

// ── Wallet badge (shown under payment method selector) ────────────────────────
// Shows available balance. Turns red when the amount being paid exceeds balance.
...
function WalletBadge({ method, wallet, paying }: { method: PayMethod; wallet: WalletBalances | null; paying?: number }) {
  ...
  return (
    <div className={`mt-1.5 flex items-center gap-1.5 px-2.5 py-1 rounded-lg border text-xs font-semibold w-fit ${insufficient ? "bg-red-50 border-red-200 text-red-700" : cfg.pill}`}>
      {cfg.icon} Available: Rs {fmt(bal)}{insufficient && <span className="ml-1 font-bold">⚠ Insufficient</span>}
    </div>
  );
}
...
function InsufficientBalanceError({ method, available, required }: { method: PayMethod; available: number; required: number }) {
  const label = PAY_CFG[method]?.label ?? method;
  return (
    <div className="flex items-start gap-3 bg-red-50 border-2 border-red-300 rounded-xl px-4 py-3 text-sm">
      <AlertCircle className="h-5 w-5 text-red-600 mt-0.5 shrink-0" />
      <div>
        <p className="font-black text-red-800">Cannot process payment — insufficient {label} balance</p>
        <p className="text-red-700 mt-0.5">
          <span className="font-semibold">Available:</span> Rs {fmt(available)} &nbsp;·&nbsp;
          <span className="font-semibold">Required:</span> Rs {fmt(required)} &nbsp;·&nbsp;
          <span className="font-semibold">Shortfall:</span> Rs {fmt(required - available)}
        </p>
        <p className="text-red-600 mt-1 text-xs">
          Reduce "Amount Paid Now" to Rs {fmt(available)} or less, then record the rest as balance due to the supplier.
        </p>
      </div>
    </div>
  );
...
export default function InventoryPage() {
  ...
  // ▶ NEW: inline error for new purchase form
  ...
  async function fetchInventory() { try { const r = await fetch("/api/admin/inventory"); const d = await r.json(); setInventory(d.inventory || []); } catch { setInventory([]); } }
  async function fetchProducts()  { try { const r = await fetch("/api/admin/products");   const d = await r.json(); setProducts(d.products || []); }   catch {} }
  async function fetchSuppliers() { try { const r = await fetch("/api/admin/suppliers");  const d = await r.json(); setSuppliers(d.suppliers || []); }  catch {} }
  async function fetchPurchases() { try { const r = await fetch("/api/admin/purchases");  const d = await r.json(); setPurchases(d.purchases || []); }  catch {} }
  async function fetchWallet()    { try { const r = await fetch("/api/admin/wallet");     const d = await r.json(); setWallet(d.wallet || null); }      catch {} }

  // ── Pagination ────────────────────────────────────────────────────────────

  const inventoryTotalPages = Math.ceil(inventory.length / ITEMS_PER_PAGE);
  const paginatedInventory  = useMemo(() => { const s = (inventoryCurrentPage - 1) * ITEMS_PER_PAGE; return inventory.slice(s, s + ITEMS_PER_PAGE); }, [inventory, inventoryCurrentPage]);
  const purchasesTotalPages = Math.ceil(purchases.length / ITEMS_PER_PAGE);
  const paginatedPurchases  = useMemo(() => { const s = (purchasesCurrentPage - 1) * ITEMS_PER_PAGE; return purchases.slice(s, s + ITEMS_PER_PAGE); }, [purchases, purchasesCurrentPage]);
  ...
  // Wallet availability for new purchase
  ...
  const insufficientNew = newWalletBal !== null && amountPaidNum > 0 && amountPaidNum > newWalletBal;

  // Wallet availability for edit purchase
  const editMethodCfg    = PAY_CFG[editMeta.paymentMethod];
  const editWalletBal    = wallet && editMethodCfg.walletKey ? (wallet[editMethodCfg.walletKey] as number) : null;
  const insufficientEdit = editWalletBal !== null && editAmountPaid > 0 && editAmountPaid > editWalletBal;
  ...
  const handleBulkPurchase = async (e: React.FormEvent) => {
    ...
    // ▶ HARD BLOCK: wallet must have sufficient funds before we even call the API
    if (insufficientNew) {
      setPurchaseError(
        `Cannot pay Rs ${fmt(amountPaidNum)} from ${newMethodCfg.label} wallet — only Rs ${fmt(newWalletBal!)} available. ` +
        `Reduce "Amount Paid Now" to Rs ${fmt(newWalletBal!)} or less and record the rest as balance due.`
      );
      return;
    }
    ...
    try {
      ...
      if (res.ok) {
        ...
        // Surface the wallet balance error (or any other API error) inline
        setPurchaseError(data.error || "Failed to create purchase");
      }
    } catch {
      setPurchaseError("Network error — could not create purchase");
    }
  };
  ...
  const handleEditSave = async () => {
    ...
    // ▶ HARD BLOCK for edit as well
    if (insufficientEdit) {
      setEditError(
        `Cannot pay Rs ${fmt(editAmountPaid)} from ${editMethodCfg.label} wallet — only Rs ${fmt(editWalletBal!)} available. ` +
        `Reduce the amount paid or record the difference as balance due.`
      );
      return;
    }
    ...
    try {
      const res = await fetch(`/api/admin/purchases/${editPurchase._id}`, {
        ...
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          supplier: editMeta.supplierId, paymentMethod: editMeta.paymentMethod,
          supplierInvoiceNo: editMeta.supplierInvoiceNo, notes: editMeta.notes,
          amountPaid: editAmountPaid, balanceDue: editBalanceDue, totalAmount: editTotalBill,
          products: valid.map(p => ({
      ...
      else { setEditError(data.message || data.error || "Failed to update"); }
    } catch { setEditError("Network error — could not update purchase"); }
  ...
  const reportTotals = useMemo(() => reportPurchases.reduce((acc, p) => ({
    total: acc.total + p.totalAmount, paid: acc.paid + p.amountPaid,
  ...
  const paginatedReport  = useMemo(() => { const s = (reportCurrentPage - 1) * REPORT_ITEMS_PER_PAGE; return reportPurchases.slice(s, s + REPORT_ITEMS_PER_PAGE); }, [reportPurchases, reportCurrentPage]);
  ...
  const renderProductRows = (
    ...
    const update = (i: number, field: keyof PurchaseProduct, val: string) => { const u = [...rows]; u[i] = { ...u[i], [field]: val }; setRows(u); };
    ...
    return (
      <div className="space-y-4">
        <div className="flex justify-between items-center">
          ...
          <Button type="button" onClick={add} variant="outline" size="sm" className="text-green-600 border-green-600"><Plus className="h-4 w-4 mr-1" /> Add Product</Button>
        </div>
        {rows.map((item, index) => {
            ...
            <Card key={index} className="p-4 border-l-4 border-l-blue-500 shadow-sm">
              <div className="grid grid-cols-12 gap-3 items-end mb-3">
                <div className="col-span-12 md:col-span-3">
                  ...
                  <select value={item.productId} onChange={e => update(index, "productId", e.target.value)} className="w-full mt-1 p-2 border rounded-md text-sm bg-white" required>
                ...
                <div className="col-span-3 md:col-span-2">
                  ...
                  <div className="flex mt-1">
                    <select value={item.taxType} onChange={e => update(index, "taxType", e.target.value)} className="p-2 border rounded-l-md bg-slate-50 text-xs font-bold border-r-0">
                      <option value="percentage">%</option><option value="fixed">Rs</option>
                    </select>
  ...
  // ── Payment section renderer (shared by new + edit forms) ─────────────────

  const renderPaymentSection = ({
    totalBillAmt, amountPaidAmt, balanceDueAmt, method, walletBal, insufficient,
    onAmountChange, error,
  }: {
    totalBillAmt: number; amountPaidAmt: number; balanceDueAmt: number;
    method: PayMethod; walletBal: number | null; insufficient: boolean;
    onAmountChange: (v: string) => void; error: string;
  }) => {
    const cfg = PAY_CFG[method];
    return (
      <>
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6 pt-4 border-t">
          <Card className={`p-4 ${insufficient ? "border-red-300 bg-red-50/30" : "border-orange-200 bg-orange-50/50"}`}>
            <h4 className={`font-bold flex items-center gap-2 mb-3 ${insufficient ? "text-red-800" : "text-orange-800"}`}>
              <WalletIcon className="h-4 w-4" /> Payment
            </h4>
            <label className="text-sm font-semibold block mb-1">Amount Paid Now (Rs)</label>
            <Input
              type="number" step="0.01" min="0" max={walletBal ?? undefined}
              value={amountPaidAmt || ""}
              onChange={e => onAmountChange(e.target.value)}
              className={`text-lg font-bold bg-white ${insufficient ? "border-red-400 focus:border-red-500" : "border-orange-300"}`}
              placeholder="0"
            />
            {walletBal !== null && amountPaidAmt > 0 && (
              <div className={`mt-3 p-3 rounded-xl border text-sm space-y-1 ${insufficient ? "bg-red-50 border-red-200" : "bg-green-50 border-green-200"}`}>
                <div className="flex justify-between"><span className="text-gray-600">{cfg.label} balance:</span><span className="font-bold">Rs {fmt(walletBal)}</span></div>
                <div className="flex justify-between"><span className="text-gray-600">After payment:</span>
                  <span className={`font-black ${walletBal - amountPaidAmt < 0 ? "text-red-600" : "text-green-700"}`}>Rs {fmt(walletBal - amountPaidAmt)}</span>
                </div>
              </div>
            )}
          </Card>
          <Card className="p-6 bg-slate-900 text-white flex flex-col justify-center space-y-3 shadow-xl">
            <div className="flex justify-between items-center"><span className="text-lg font-bold">Total Bill:</span><span className="text-2xl font-bold">Rs {fmt(totalBillAmt)}</span></div>
            <div className="flex justify-between items-center opacity-80 text-sm border-t border-slate-700 pt-2"><span>Amount Paid:</span><span className="text-green-400">(-) Rs {fmt(amountPaidAmt)}</span></div>
            <div className="flex justify-between items-center pt-2 border-t border-slate-700">
              <span className="text-lg font-bold">Balance Due:</span>
              <span className={`text-3xl font-black ${balanceDueAmt > 0 ? "text-red-400" : "text-green-400"}`}>Rs {fmt(Math.abs(balanceDueAmt))}</span>
            </div>
          </Card>
        </div>

        {/* ▶ Inline error — shown when wallet is insufficient OR API returns error */}
        {(insufficient || error) && (
          <InsufficientBalanceError
            method={method}
            available={walletBal ?? 0}
            required={amountPaidAmt}
          />
        )}
        {!insufficient && error && (
        ...
        <div className="flex flex-col gap-4 no-print">
          <div className="flex justify-between items-start flex-wrap gap-4">
            ...
            <div className="flex gap-2 flex-wrap">
              ...
              <Dialog open={isAddDialogOpen} onOpenChange={open => { setIsAddDialogOpen(open); if (!open) setPurchaseError(""); }}>
                ...
                <DialogContent className="max-w-6xl max-h-[95vh] overflow-y-auto">
                  ...
                  <form onSubmit={handleBulkPurchase} className="space-y-6 mt-4">
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                        ...
                        <div>
                          ...
                          <select value={purchaseData.paymentMethod}
                            onChange={e => { setPurchaseData({ ...purchaseData, paymentMethod: e.target.value as PayMethod }); setPurchaseError(""); }}
                    ...
                    {renderPaymentSection({
                      totalBillAmt:   totalBill,
                      amountPaidAmt:  amountPaidNum,
                      balanceDueAmt:  balanceDue,
                      method:         purchaseData.paymentMethod,
                      walletBal:      newWalletBal,
                      insufficient:   insufficientNew,
                      onAmountChange: v => { setPurchaseData({ ...purchaseData, amountPaid: v }); setPurchaseError(""); },
                      error:          purchaseError,
                    })}

                    <Button
                      type="submit"
                      disabled={insufficientNew}
                      className={`w-full py-6 text-xl font-bold shadow-xl transition-all ${insufficientNew ? "bg-gray-400 cursor-not-allowed opacity-60" : "bg-green-600 hover:bg-green-700"}`}
                    >
                      {insufficientNew
                        ? `⚠ Insufficient ${newMethodCfg.label} balance — adjust payment`
                ...
                {isLoading ? (
                  ...
                  <table className="w-full text-left">
                    <thead className="bg-slate-50 border-b-2">
                      <tr>{["Product","SKU","Purchased","Sold","In Stock","Selling Price","Profit/Unit","Status","Batches"].map(h => <th key={h} className="px-6 py-4 font-bold text-gray-600">{h}</th>)}</tr>
                    </thead>
                        ...
                        return (
                          <React.Fragment key={item._id}>
                            <tr className={`transition-colors ${isSoldOut ? "bg-red-50/40 hover:bg-red-50" : "hover:bg-slate-50"}`}>
                              ...
                              <td className="px-6 py-4"><span className="text-lg font-bold text-blue-700">{totalPurchased}</span><span className="text-xs text-gray-400 ml-1">units</span></td>
                              <td className="px-6 py-4">
                                ...
                                {totalPurchased > 0 && <div className="mt-1 w-16 h-1.5 bg-gray-200 rounded-full overflow-hidden"><div className="h-full bg-indigo-500 rounded-full" style={{ width: `${Math.min((totalSold/totalPurchased)*100,100)}%` }} /></div>}
                              </td>
                              <td className="px-6 py-4"><span className={`text-lg font-bold ${isSoldOut ? "text-red-600" : isLow ? "text-orange-600" : "text-green-600"}`}>{currentStock}</span><span className="text-xs text-gray-400 ml-1">units</span></td>
                              ...
                              <td className="px-6 py-4">
                                {isSoldOut ? <span className="px-3 py-1 rounded-full text-xs font-bold uppercase bg-red-100 text-red-700 flex items-center gap-1 w-fit"><PackageX className="h-3 w-3" /> Sold Out</span>
                                  : isLow  ? <span className="px-3 py-1 rounded-full text-xs font-bold uppercase bg-orange-100 text-orange-700 flex items-center gap-1 w-fit"><AlertTriangle className="h-3 w-3" /> Low</span>
                            ...
                            {expandedInventory.has(item._id) && (
                              <tr><td colSpan={9} className="px-6 py-4 bg-gray-50">
                                      ...
                                      return (
                                        <div key={batch._id} className={`p-3 rounded-lg border-2 ${batch.status === "active" ? "bg-green-50 border-green-300" : batch.status === "partial" ? "bg-yellow-50 border-yellow-300" : "bg-gray-100 border-gray-300"}`}>
                                          <div className="grid grid-cols-9 gap-3 text-sm">
                                            ...
                                            <div><span className="text-gray-600 text-xs">Remaining</span><p className={`font-bold ${(batch.remainingQuantity??0)===0?"text-red-600":"text-green-600"}`}>{batch.remainingQuantity??0}</p><div className="mt-1 w-12 h-1 bg-gray-200 rounded-full overflow-hidden"><div className="h-full bg-indigo-400 rounded-full" style={{width:`${soldPct}%`}}/></div></div>
                                            ...
                                            <div><span className="text-gray-600 text-xs">Status</span><p className={`font-semibold capitalize text-xs mt-0.5 px-2 py-0.5 rounded-full w-fit ${batch.status==="active"?"bg-green-200 text-green-800":batch.status==="partial"?"bg-yellow-200 text-yellow-800":"bg-gray-200 text-gray-700"}`}>{batch.status}</p></div>
                                          </div>
                                        </div>
                                      );
                                    })}
          ...
          <Card className="shadow-xl overflow-hidden border-0">
            <div className="overflow-x-auto">
              {paginatedPurchases.length > 0 ? (
                <table className="w-full text-left">
                  <thead className="bg-slate-50 border-b-2">
                    <tr>{["Date","Invoice","Supplier","Items","Total","Paid","Balance","Method","Status","Actions"].map(h => <th key={h} className="px-5 py-4 font-bold text-gray-600 text-sm">{h}</th>)}</tr>
                  </thead>
                  <tbody className="divide-y">
                    {paginatedPurchases.map(p => {
                      ...
                      return (
                        <React.Fragment key={p._id}>
                          ...
                          {isExpanded && (
                            <tr><td colSpan={10} className="px-5 py-3 bg-blue-50/50">
                              <div className="rounded-xl overflow-hidden border border-blue-100">
                                <table className="w-full text-sm">
                                  <thead className="bg-blue-100/60"><tr>{["Product","SKU","Qty","Base Rate","Tax","Freight","Unit Cost","Selling","Profit/Unit","Subtotal"].map(h => <th key={h} className="px-4 py-2 text-left text-xs font-bold text-blue-800">{h}</th>)}</tr></thead>
                                  ...
                                  <tfoot className="bg-blue-100/40"><tr><td colSpan={9} className="px-4 py-2 font-bold text-right text-gray-700">Total:</td><td className="px-4 py-2 font-black text-gray-900">Rs {fmt(p.totalAmount)}</td></tr></tfoot>
                                </table>
                              </div>
          ...
          <div className="space-y-5">
              ...
              <div className="flex items-center gap-3 flex-wrap">
                ...
                {showCustomDate && (
                  <div className="flex items-end gap-2">
                    <div><label className="text-xs font-bold text-gray-500 block mb-1">FROM</label><input type="date" value={reportFrom} onChange={e=>setReportFrom(e.target.value)} className="px-3 py-1.5 border border-gray-200 rounded-lg text-sm"/></div>
                    <div><label className="text-xs font-bold text-gray-500 block mb-1">TO</label><input type="date" value={reportTo} onChange={e=>setReportTo(e.target.value)} className="px-3 py-1.5 border border-gray-200 rounded-lg text-sm"/></div>
                  </div>
                )}
              </div>
              <Button onClick={() => window.print()} variant="outline" className="gap-2 rounded-xl border-gray-300 no-print"><Printer className="h-4 w-4"/> Print Report</Button>
            </div>
            ...
            <div id="inv-print-area">
              ...
              <div className="grid grid-cols-2 md:grid-cols-5 gap-4 mb-6">
                {[
                  { label:"Orders",          value: reportTotals.orders,                         color:"bg-slate-50 border-slate-200 text-slate-700",  icon:<FileText className="h-5 w-5"/> },
                  { label:"Total Purchased",  value:`Rs ${fmt(reportTotals.total)}`,              color:"bg-blue-50 border-blue-200 text-blue-800",      icon:<ShoppingBag className="h-5 w-5"/> },
                  { label:"Amount Paid",      value:`Rs ${fmt(reportTotals.paid)}`,               color:"bg-green-50 border-green-200 text-green-800",   icon:<Banknote className="h-5 w-5"/> },
                  { label:"Balance Due",      value:`Rs ${fmt(reportTotals.due)}`,                color:reportTotals.due>0?"bg-red-50 border-red-200 text-red-800":"bg-gray-50 border-gray-200 text-gray-500", icon:<AlertCircle className="h-5 w-5"/> },
                  { label:"Items Purchased",  value:reportTotals.items.toLocaleString(),         color:"bg-indigo-50 border-indigo-200 text-indigo-800", icon:<Package className="h-5 w-5"/> },
                ].map((s,i) => (
              ...
              {supplierSummary.length > 0 && (
                <Card className="border-0 shadow-md overflow-hidden mb-6">
                  <div className="bg-gray-900 text-white px-5 py-3 flex items-center gap-2">
                    <Building2 className="h-4 w-4"/><h3 className="font-black">By Supplier — {periodLabel}</h3>
                    <span className="ml-auto text-xs opacity-60">{supplierSummary.length} suppliers</span>
                  </div>
                  <div className="overflow-x-auto">
                    <table className="w-full text-sm">
                      <thead className="bg-gray-50 border-b"><tr>{["Supplier","Orders","Items","Total Amount","Amount Paid","Balance Due","Status"].map(h=><th key={h} className="px-5 py-3 text-left text-xs font-bold text-gray-500 uppercase">{h}</th>)}</tr></thead>
                      <tbody className="divide-y">
              ...
              <Card className="border-0 shadow-md overflow-hidden">
                <div className="bg-slate-800 text-white px-5 py-3 flex items-center justify-between">
                  <div className="flex items-center gap-2"><Receipt className="h-4 w-4"/><h3 className="font-black">All Purchase Orders — {periodLabel}</h3></div>
                  <span className="text-xs opacity-60">{reportPurchases.length} orders · 50 per page</span>
                </div>
                {reportPurchases.length === 0 ? (
                  <div className="text-center py-16"><ShoppingBag className="h-12 w-12 text-gray-300 mx-auto mb-3"/><p className="text-gray-400 font-semibold">No purchases this period</p></div>
                ) : (
                  <>
                    <div className="overflow-x-auto">
                      <table className="w-full text-sm">
                        <thead className="bg-gray-50 border-b-2 border-gray-200">
                          <tr>
                            {["#","Date & Time","Invoice","Supplier","Items","Total","Paid","Balance","Method","Status","Details"].map(h=>(
                              <th key={h} className={`px-4 py-3 text-left text-xs font-bold text-gray-500 uppercase${h==="Details"?" no-print":""}`}>{h}</th>
                            ))}
                          </tr>
                        </thead>
                            ...
                            return (
                              <React.Fragment key={p._id}>
                                <tr className={`transition-colors ${isExpanded?"bg-blue-50/30":"hover:bg-gray-50"}`}>
                                  ...
                                  <td className="px-4 py-3"><p className="text-sm font-semibold text-gray-800 whitespace-nowrap">{fmtDate(p.createdAt)}</p><p className="text-xs text-gray-400">{new Date(p.createdAt).toLocaleTimeString("en-PK",{hour:"2-digit",minute:"2-digit",hour12:true})}</p></td>
                                  <td className="px-4 py-3"><div className="flex items-center gap-1"><Hash className="h-3 w-3 text-gray-400"/><span className="font-mono text-xs text-gray-700">{p.supplierInvoiceNo||<span className="text-gray-300 italic">—</span>}</span></div></td>
                                  <td className="px-4 py-3"><div className="flex items-center gap-2"><div className="w-7 h-7 rounded-full bg-blue-100 text-blue-700 flex items-center justify-center text-xs font-black shrink-0">{p.supplier.name.charAt(0).toUpperCase()}</div><span className="font-semibold text-gray-900 whitespace-nowrap">{p.supplier.name}</span></div></td>
                                  <td className="px-4 py-3"><p className="text-sm font-bold text-gray-900">{totalQty} units</p><p className="text-xs text-gray-400 max-w-[140px] truncate">{p.products.map(pr=>pr.product?.name).filter(Boolean).join(", ")}</p></td>
                                  ...
                                  <td className="px-4 py-3 whitespace-nowrap"><span className={`font-bold ${p.balanceDue>0?"text-red-600":"text-gray-400"}`}>{p.balanceDue>0?`Rs ${fmt(p.balanceDue)}`:"—"}</span></td>
                                  <td className="px-4 py-3">{mCfg?<span className={`inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-xs font-bold border ${mCfg.pill}`}>{mCfg.icon}{mCfg.label}</span>:<span className="text-gray-500 text-xs">{p.paymentMethod}</span>}</td>
                                  <td className="px-4 py-3"><span className={`px-2.5 py-1 rounded-full text-xs font-bold uppercase ${p.paymentStatus==="completed"?"bg-green-100 text-green-700":p.paymentStatus==="partial"?"bg-yellow-100 text-yellow-700":"bg-red-100 text-red-700"}`}>{p.paymentStatus}</span></td>
                                  <td className="px-4 py-3 no-print">
                                    <button onClick={() => { const s=new Set(expandedReportRows); s.has(p._id)?s.delete(p._id):s.add(p._id); setExpandedReportRows(s); }}
                                      ...
                                      {isExpanded?<ChevronDown className="h-3.5 w-3.5"/>:<ChevronRight className="h-3.5 w-3.5"/>}{isExpanded?"Hide":"View"}
                                    </button>
                                  </td>
                                </tr>
                                {isExpanded && (
                                  <tr><td colSpan={11} className="px-4 py-0 bg-blue-50/40">
                                    <div className="py-3 pl-10">
                                      <div className="rounded-xl border border-blue-200 overflow-hidden shadow-sm">
                                        <div className="bg-blue-100/80 px-4 py-2 flex items-center justify-between">
                                          <span className="text-xs font-black text-blue-800 uppercase tracking-wide">Purchase Items — {p.supplier.name} · {fmtDate(p.createdAt)}</span>
                                          {p.supplierInvoiceNo&&<span className="text-xs font-mono text-blue-600 bg-white px-2 py-0.5 rounded border border-blue-200">INV #{p.supplierInvoiceNo}</span>}
                                        </div>
                                        <table className="w-full text-sm bg-white">
                                          <thead className="bg-blue-50 border-b border-blue-100"><tr>{["Product","SKU","Qty","Base Rate","Tax","Freight","Unit Cost","Selling Price","Profit/Unit","Line Total"].map(h=><th key={h} className="px-4 py-2 text-left text-xs font-bold text-blue-700">{h}</th>)}</tr></thead>
                                            ...
                                            {p.products.map((item,idx)=>{
                                              ...
                                              return (
                                                <tr key={idx} className="hover:bg-blue-50/30">
                                                  ...
                                                  <td className="px-4 py-2.5 text-blue-600">{item.taxType==="percent"||item.taxType==="percentage"?`${item.taxValue??0}%`:`Rs ${fmt(item.taxValue??0)}`}</td>
                                                  ...
                                                  <td className={`px-4 py-2.5 font-bold ${profit>=0?"text-green-600":"text-red-600"}`}>Rs {fmt(profit)}<span className="text-xs font-normal ml-1 opacity-60">({item.unitCostWithTax>0?((profit/item.unitCostWithTax)*100).toFixed(1):0}%)</span></td>
                                          ...
                                          <tfoot className="bg-gray-50 border-t-2 border-gray-200">
                                            <tr><td colSpan={2} className="px-4 py-2.5">{p.notes&&<span className="text-xs text-gray-500 italic">Note: {p.notes}</span>}</td><td className="px-4 py-2.5 font-black">{p.products.reduce((s,pr)=>s+pr.quantity,0)}</td><td colSpan={5}/><td className="px-4 py-2.5 text-xs text-gray-500 font-bold">Total Cost:</td><td className="px-4 py-2.5 font-black text-gray-900">Rs {fmt(p.totalAmount)}</td></tr>
                                            <tr className="bg-slate-800 text-white text-xs">
                                              <td colSpan={5} className="px-4 py-2 font-semibold">Paid: <span className="text-green-400 font-black">Rs {fmt(p.amountPaid)}</span>{p.balanceDue>0&&<span className="ml-4 text-red-400">Balance Due: <span className="font-black">Rs {fmt(p.balanceDue)}</span></span>}</td>
                                              <td colSpan={5} className="px-4 py-2 text-right font-semibold">Status: <span className={`font-black ${p.paymentStatus==="completed"?"text-green-400":p.paymentStatus==="partial"?"text-yellow-400":"text-red-400"}`}>{p.paymentStatus.toUpperCase()}</span><span className="ml-4 opacity-60">via {PAY_CFG[p.paymentMethod as PayMethod]?.label||p.paymentMethod}</span></td>
                                            </tr>
                                          </tfoot>
                                        </table>
                                      </div>
                        ...
                        {reportPurchases.length > 0 && (
                          <tfoot className="bg-slate-900 text-white text-sm">
                            <tr>
                              <td colSpan={4} className="px-4 py-3 font-black text-slate-300 uppercase text-xs tracking-wide">Period Total ({reportPurchases.length} orders)</td>
                              ...
                              <td className={`px-4 py-3 font-black ${reportTotals.due>0?"text-red-400":"text-slate-400"}`}>{reportTotals.due>0?`Rs ${fmt(reportTotals.due)}`:"—"}</td>
                              <td colSpan={3}/>
                            </tr>
                          </tfoot>
                        )}
                    ...
                    <Pagination currentPage={reportCurrentPage} totalPages={reportTotalPages} onPageChange={setReportCurrentPage} totalItems={reportPurchases.length} itemsPerPage={REPORT_ITEMS_PER_PAGE} label="purchase orders"/>
                  </>
                )}
              </Card>
            </div>
          ...
          <div className="fixed inset-0 z-50 bg-black/60 flex items-start justify-center p-4 overflow-y-auto">
            <div className="bg-white rounded-2xl shadow-2xl w-full max-w-6xl my-8">
              <div className="flex items-center justify-between px-6 py-5 border-b bg-blue-50 rounded-t-2xl">
                <h2 className="text-xl font-black text-gray-900 flex items-center gap-2"><Pencil className="h-5 w-5 text-blue-600"/> Edit Purchase — {editPurchase.supplier.name}</h2>
                <button onClick={() => setEditPurchase(null)} className="text-gray-400 hover:text-gray-600 p-1 rounded-lg hover:bg-gray-100"><X className="h-5 w-5"/></button>
              </div>
              <div className="p-6 space-y-6">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
                    <div>
                      ...
                      <select value={editMeta.supplierId} onChange={e=>setEditMeta({...editMeta,supplierId:e.target.value})} className="w-full p-2 border rounded-md bg-white text-sm">
                        <option value="">Choose...</option>{suppliers.map(s=><option key={s._id} value={s._id}>{s.name}</option>)}
                      </select>
                    </div>
                    <div>
                    ...
                    <div>
                      ...
                      <select value={editMeta.paymentMethod} onChange={e=>{setEditMeta({...editMeta,paymentMethod:e.target.value as PayMethod});setEditError("");}} className="w-full p-2 border rounded-md bg-white text-sm">
                        {(Object.entries(PAY_CFG) as [PayMethod, typeof PAY_CFG[PayMethod]][]).map(([k,c])=><option key={k} value={k}>{c.label}</option>)}
                      </select>
                ...
                {renderProductRows(editProducts, setEditProducts)}

                {renderPaymentSection({
                  totalBillAmt:   editTotalBill,
                  amountPaidAmt:  editAmountPaid,
                  balanceDueAmt:  editBalanceDue,
                  method:         editMeta.paymentMethod,
                  walletBal:      editWalletBal,
                  insufficient:   insufficientEdit,
                  onAmountChange: v => { setEditMeta({...editMeta, amountPaid: v}); setEditError(""); },
                  error:          editError,
                })}

                <div className="flex gap-3">
                  <Button variant="outline" onClick={() => setEditPurchase(null)} className="flex-1 rounded-xl">Cancel</Button>
                  <Button onClick={handleEditSave} disabled={isEditSaving || insufficientEdit}
                    className={`flex-1 rounded-xl py-3 text-base font-bold transition-all ${insufficientEdit?"bg-gray-400 cursor-not-allowed opacity-60":"bg-blue-600 hover:bg-blue-700"}`}>
                    {isEditSaving ? "Saving…" : insufficientEdit ? `⚠ Insufficient ${editMethodCfg.label} balance` : "Save All Changes & Update Stock/Wallet"}
                  </Button>
                </div>
              </div>
            </div>
