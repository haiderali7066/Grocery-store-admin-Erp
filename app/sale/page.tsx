...
import { Zap, Clock, ShoppingCart, Flame, AlertCircle, ArrowRight } from "lucide-react";
    ...
    >
      {label && (
        <div className="absolute top-0 left-0 z-10">
          <div className="bg-yellow-400 text-gray-900 text-[10px] font-black px-3 py-1 rounded-br-xl tracking-widest uppercase shadow-sm">{label}</div>
        </div>
      )}
      ...
      {outOfStock && (
        <div className="absolute inset-0 z-20 bg-white/80 backdrop-blur-sm flex items-center justify-center">
          <span className="bg-gray-900 text-white text-xs font-black px-5 py-2 rounded-full tracking-widest uppercase">Sold Out</span>
        </div>
      )}
      <div className="relative h-44 bg-gradient-to-br from-gray-50 to-gray-100 overflow-hidden">
        <Image src={product.mainImage || "/placeholder.svg"} alt={product.name} fill className="object-contain p-5 group-hover:scale-[1.06] transition-transform duration-500" />
      </div>
      <div className="p-4 flex flex-col flex-1">
        <p className="text-[10px] font-black text-green-700 uppercase tracking-widest mb-1">Flash Deal</p>
        <h3 className="font-bold text-gray-900 text-sm leading-snug mb-1 line-clamp-2 flex-1">{product.name}</h3>
        <p className="text-xs text-gray-400 mb-3">{product.unitSize} {product.unitType}</p>
        {lowStock && (
          <div className="mb-3">
            <span className="text-[10px] font-bold text-orange-500 uppercase tracking-wide">🔥 Only {product.stock} left</span>
            <div className="h-1.5 bg-gray-100 rounded-full overflow-hidden mt-1">
              <div className="h-full bg-gradient-to-r from-yellow-400 to-orange-400 rounded-full" style={{ width: `${stockPct}%` }} />
            </div>
          </div>
        )}
        <div className="flex items-baseline gap-2 mb-4">
          <span className="text-xl font-black text-green-700">Rs. {salePrice.toFixed(0)}</span>
          {product.discount > 0 && <span className="text-xs text-gray-400 line-through">Rs. {product.retailPrice.toFixed(0)}</span>}
        </div>
        <button
          onClick={() => !outOfStock && addItem({ id: product._id, name: product.name, price: salePrice, quantity: 1, image: product.mainImage, weight: `${product.unitSize}${product.unitType}` })}
          disabled={outOfStock}
    ...
    <div className="min-h-screen bg-[#f9fafb] flex flex-col">
      ...
      <main className="flex-1 max-w-7xl mx-auto w-full px-4 sm:px-6 lg:px-8 pb-20 -mt-2">
        <div className="flex items-center gap-4 mb-8">
          ...
          {!isLoading && <span className="text-xs font-semibold text-gray-400">{products.length} deal{products.length !== 1 ? "s" : ""}</span>}
        </div>
