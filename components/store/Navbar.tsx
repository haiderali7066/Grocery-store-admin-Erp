...
export function Navbar() {
  ...
  return (
    <nav className="sticky top-0 z-50 w-full border-b border-secondary bg-background shadow-sm">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex justify-between items-center h-16 md:h-20">
          {/* Logo */}
          <Link href="/" className="flex items-center gap-2 text-lg md:text-xl font-bold text-primary">
          ...
          <div className="hidden md:flex items-center gap-4 lg:gap-6">
            <Link href="/products" className="text-foreground hover:text-primary font-medium text-sm transition-colors">
              Products
            </Link>
        ...
        {isMenuOpen && (
          <div className="md:hidden pb-4 border-t border-secondary">
            <div className="py-3">
              <Input type="search" placeholder="Search products..." className="mb-3 rounded-full bg-secondary border-0" />
            </div>
            <Link href="/products" className="block py-2 text-foreground hover:text-primary transition-colors">
              Products
            </Link>
            <Link href="/cart" className="block py-2 text-foreground hover:text-primary transition-colors relative">
              Cart {cartItemCount > 0 && <span className="ml-1 bg-primary text-primary-foreground text-xs font-bold px-2 py-0.5 rounded-full">{cartItemCount > 99 ? '99+' : cartItemCount}</span>}
            </Link>
            {isAuthenticated && user ? (
              <>
                <Link href="/account" className="block py-2 text-foreground hover:text-primary transition-colors">
                  My Account
                </Link>
                <Link href="/orders" className="block py-2 text-foreground hover:text-primary transition-colors">
                  My Orders
                </Link>
                {user.role === 'admin' && (
                  <Link href="/admin" className="block py-2 text-foreground hover:text-primary transition-colors">
                    Admin Panel
                  </Link>
                )}
                <button
